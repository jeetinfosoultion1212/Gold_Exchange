...
/**
 * Signed cash/bank ledger delta for an Exchange due_amount (mirrors ge_signed_due_delta()
 * in exchange.php). due_amount is stored as a positive "still outstanding" figure, but a
 * negative difference_weight means the SHOP owes the CUSTOMER, so it must reduce (not add
 * to) the party's balance.
 */
function msc_signed_due_delta(float $due_amount, float $difference_weight): float
{
...
try {
    ...
    switch ($action) {
        ...
        case 'update_exchange_fine':
            ...
            if ($party_id > 0) {
                $old_signed_due = msc_signed_due_delta($old_due, $old_diff);
                $new_signed_due = msc_signed_due_delta($new_due, $new_diff);
                msc_apply_party_exchange_delta($conn, $party_id, -$old_signed_due, -$old_diff, $payment_method, $is_silver);
                msc_apply_party_exchange_delta($conn, $party_id, $new_signed_due, $new_diff, $payment_method, $is_silver);
            }
