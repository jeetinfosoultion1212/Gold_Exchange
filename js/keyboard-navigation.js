 ...
 * Provides full keyboard accessibility for the booking form
 */

const KeyboardNavigation = (() => {
    // Field order as per requirements (readonly fields are included but will be skipped during navigation)
    const FIELD_ORDER = [
        'bookingIdInput',      // 1. Booking ID (readonly, but focusable)
        'date_of_transaction', // 2. Date
        'partyNameInput',      // 3. Party Name
        'booking_weight',      // 4. Weight
        'purity',              // 5. Purity
        'rate',                // 6. Rate
        'total_amount',        // 7. Total (readonly, skip in navigation)
        'booking_type',        // 8. Booking Type
        'narration',           // 9. Narration
    ];
    
    // Fields that should be skipped during Enter navigation (readonly/calculated fields)
    const SKIP_FIELDS = ['total_amount'];
    ...
    let dropdownStates = {};
    let fieldInteracted = {}; // Track if user has interacted with each field
    ...
    function init() {
        ...
        Object.keys(fields).forEach(fieldName => {
            ...
            // Track user interaction - only mark as interacted when user actually types/changes value
            // Don't mark on focus alone, as focus can happen on page load
            field.addEventListener('blur', (e) => {
                // Only validate if user has interacted with the field (typed something)
                if (fieldInteracted[fieldName]) {
            ...
            // For select fields, mark as interacted when value changes
        ...
        if (submitBtn) {
            submitBtn.addEventListener('keydown', (e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                    ...
                    e.preventDefault();
                    const lastFieldName = FIELD_ORDER[FIELD_ORDER.length - 1];
    ...
    function handleKeyDown(e, fieldName) {
        ...
        if (e.key === 'Enter' && !e.shiftKey) {
            ...
            // For readonly fields (like Booking ID), just move to next field
            if (field.readOnly) {
                e.preventDefault();
            ...
            // For party name with dropdown - let PartySearch handle it when dropdown is open
            if (fieldName === 'partyNameInput') {
                const partyList = document.getElementById('partyList');
                if (partyList && !partyList.classList.contains('hidden')) {
                    // Dropdown is open, let PartySearch handle ALL keys (Enter, Arrow keys, etc.)
                    // Don't prevent default or move to next field - PartySearch will handle it
                    return; // Don't interfere - let PartySearch handle it completely
                }
                // If dropdown is closed and Enter is pressed, check if we should search or validate
                const partyId = document.getElementById('partyId');
                if (!partyId || !partyId.value) {
                    // Party not selected yet - let PartySearch handle Enter to trigger search
                    // Don't prevent default here, let PartySearch handle it
                    return;
                }
                // Party is already selected, so we can move to next field
                // Validate first
                fieldInteracted[fieldName] = true;
                if (!validateField(fieldName, field)) {
                    e.preventDefault();
                    return; // Stay in field if validation fails
                }
            }
    ...
    function moveToPreviousField(currentFieldName) {
        ...
        if (currentIndex === -1) {
            ...
            if (lastField && !lastField.disabled) {
                // Skip readonly fields that are not focusable (like total_amount)
                if (SKIP_FIELDS.includes(lastFieldName)) {
        ...
        for (let i = currentIndex - 1; i >= 0; i--) {
            const prevFieldName = FIELD_ORDER[i];
            
            // Skip fields that are in the skip list
            if (SKIP_FIELDS.includes(prevFieldName)) {
                continue;
            }
            
            const prevField = fields[prevFieldName];
            
            // Allow readonly fields if they're focusable (like bookingIdInput)
        ...
        // or optionally wrap to submit button
    ...
    /**
     * Open dropdown (for custom dropdowns, native selects work differently)
     */
    function openDropdown(fieldName) {
        const field = fields[fieldName];
        if (!field || field.tagName !== 'SELECT') return;

        // For native select, we simulate opening by focusing
    ...
    function validateField(fieldName, field) {
        ...
        if (fieldName === 'partyNameInput') {
            // Don't show error on page load - only after user interaction
            if (!fieldInteracted[fieldName]) {
                return true; // Skip validation if not interacted
            }
            const partyId = document.getElementById('partyId');
            if (!partyId || !partyId.value) {
                isValid = false;
        ...
        if (fieldName === 'booking_type' && !field.value) {
            isValid = false;
    ...
    function clearValidationError(fieldName) {
        ...
        // Also check parent element for error container (in case it's in a different structure)
        const parentErrorContainer = field.closest('div')?.querySelector('.validation-error');
        if (parentErrorContainer && parentErrorContainer !== errorContainer) {
            parentErrorContainer.textContent = '';
            parentErrorContainer.classList.add('hidden');
        }
    }
