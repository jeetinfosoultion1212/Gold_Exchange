            ...
            <div class="bg-white rounded-lg shadow-md border border-gray-200" style="flex: 0 0 55%;">
                ...
                <form id="exchangeForm" onsubmit="return false;" class="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div class="relative">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" class="block w-full pl-10 pr-3 py-2.5 text-sm font-medium border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 shadow-sm" name="receipt_id" id="receiptId" placeholder="Search..." autocomplete="off">
                            </div>
                    ...
                    <div class="p-4 grid grid-cols-2 lg:grid-cols-5 gap-3">
                        <!-- Received Weight -->
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1">Received Wt (g)</label>
                            <div class="relative">
                                <span class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    <i class="fas fa-arrow-down text-green-500 text-sm"></i>
                                </span>
                                <input type="number" step="0.001" class="block w-full pl-10 pr-3 py-2.5 text-sm font-bold text-gray-900 border border-gray-300 rounded-md focus:ring-yellow-500 focus:border-yellow-500 shadow-sm bg-green-100" name="received_weight" id="receivedWeight" required placeholder="0.000">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" step="0.01" class="block w-full pl-10 pr-3 py-2.5 text-sm font-bold border border-gray-300 rounded-md focus:ring-yellow-500 focus:border-yellow-500 shadow-sm" name="purity" id="purity" placeholder="0.00" required>
                            </div>
                        </div>

                         <!-- Fine Weight -->
                         <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1">Fine Wt (g)</label>
                            <div class="relative">
                                <span class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    <i class="fas fa-gem text-yellow-600 text-sm"></i>
                                </span>
                                <input type="number" step="0.001" class="block w-full pl-10 pr-3 py-2.5 text-sm font-bold text-gray-500 bg-gray-100 border border-gray-300 rounded-md shadow-sm cursor-not-allowed" name="fine_weight" id="fineWeight" readonly placeholder="0.000">
                            </div>
                        </div>

                        <!-- Issue Weight -->
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1">Issue Wt (g)</label>
                            <div class="relative">
                                <span class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    <i class="fas fa-arrow-up text-red-500 text-sm"></i>
                                </span>
                                <input type="number" step="0.001" class="block w-full pl-10 pr-3 py-2.5 text-sm font-bold text-gray-900 border border-gray-300 rounded-md focus:ring-yellow-500 focus:border-yellow-500 shadow-sm bg-red-100" name="issue_weight" id="issueWeight" required placeholder="0.000">
                            </div>
                        </div>

                        <!-- Difference -->
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1">Difference</label>
                            <div class="relative">
                                <span class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-400">
                                    <i class="fas fa-exchange-alt text-sm"></i>
                                </span>
                                <input type="text" class="block w-full pl-10 pr-3 py-2.5 text-sm font-bold text-gray-700 bg-gray-100 border border-gray-300 rounded-md shadow-sm" id="differenceWeight" readonly placeholder="0.000">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left text-gray-500">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                                <tr>
                                    <th class="text-left py-2 px-2 font-bold w-16"># / ID</th>
                                    <th class="text-left py-2 px-2 font-bold">Party</th>
                                    <th class="text-left py-2 px-2 font-bold">R.Wt / Purity</th>
                                    <th class="text-left py-2 px-2 font-bold">Fine / Rate</th>
                                    <th class="text-left py-2 px-2 font-bold">I.Wt / Diff</th>
                                    <th class="text-left py-2 px-2 font-bold">Amt</th>
                                    <th class="text-left py-2 px-2 font-bold">Payment</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100" id="recentTransactionList">
                                ...
                                ?>
                                    <tr class="hover:bg-gray-50 transition border-b border-gray-100">
                                        <!-- Col 1: Serial + ID + Date (Clickable for Print) -->
                                        <td class="py-2 px-2 align-top group cursor-pointer print-receipt"
                                            data-id="<?= $t['id'] ?>"
                                            data-receipt-id="<?= htmlspecialchars($t['receipt_id']) ?>"
                                            data-party="<?= htmlspecialchars($t['party_name']) ?>"
                                            data-date="<?= $t['date_of_transaction'] ?>"
                                            data-received="<?= $t['received_weight'] ?>"
                                            data-purity="<?= $t['purity'] ?>"
                                            data-fine="<?= $t['fine_weight'] ?>"
                                            data-issue="<?= $t['delivered_weight'] ?>"
                                            data-diff="<?= $t['difference_weight'] ?>"
                                            data-rate="<?= $t['rate'] ?>"
                                            ...
                                            <div class="font-bold text-blue-600 group-hover:text-blue-800 hover:underline text-xs">
                                                #<?= $serial ?> <?= htmlspecialchars($t['receipt_id']) ?>
                                            </div>
                                            <div class="text-[10px] text-gray-400 mt-0.5"><?= date('d M', strtotime($t['date_of_transaction'])) ?></div>
                                        </td>

                                        <!-- Col 2: Party -->
                                        <td class="py-2 px-2 align-top font-bold text-gray-700 text-xs text-wrap max-w-[100px]">
                                            <?= htmlspecialchars($t['party_name']) ?>
                                        </td>

                                        <!-- Col 3: Rec. Wt / Purity (Rec Wt ON TOP) -->
                                        <td class="py-2 px-2 align-top">
                                            <div class="font-bold text-gray-800 text-xs"><?= number_format($t['received_weight'], 3) ?>g</div>
                                            <div class="text-[10px] text-gray-500 mt-0.5"><?= number_format($t['purity'], 2) ?>%</div>
                                        </td>

                                        <!-- Col 4: Fine Wt / Rate (Fine Wt ON TOP) -->
                                        <td class="py-2 px-2 align-top">
                                            <div class="text-xs font-bold text-yellow-600">F: <?= number_format($t['fine_weight'], 3) ?>g</div>
                                            <div class="text-[10px] text-gray-500 mt-0.5">₹<?= number_format($t['rate'], 2) ?></div>
                                        </td>

                                        <!-- Col 5: I.Wt / Diff (Issue Wt ON TOP) -->
                                        <td class="py-2 px-2 align-top">
                                            <div class="font-bold text-xs text-gray-800"><?= number_format($t['delivered_weight'], 3) ?>g</div>
                                            <div class="text-[10px] font-bold <?= $diffColor ?> mt-0.5">
                                                Diff: <?= number_format($t['difference_weight'], 3) ?>g
                                            </div>
                                        </td>

                                        <!-- Col 6: Amt (Total Bill) -->
                                        <td class="py-2 px-2 align-top">
                                            <div class="font-bold text-gray-700 text-xs">₹<?= number_format($t['amount'], 0) ?></div>
                                            <div class="text-[10px] text-gray-500">
                                                <?php 
                                                    // Dynamically calculate status to ensure accuracy
                                                    if ($t['payment_amount'] >= $t['amount']) {
                                                        echo '<span class="text-green-600 font-bold">Paid</span>';
                                                    } else {
                                                        echo '<span class="text-red-500 font-bold">Due</span>';
                                                    }
                                                ?>
                                            </div>
                                        </td>
                                        ...
                                        <!-- Col 7: Payment (Flow) -->
                                        <td class="py-2 px-2 align-top">
                                            <div class="text-xs"><?= $payDisplay ?></div>
                                        </td>
                                    </tr>
                                <?php endforeach; 
                                else: ?>
