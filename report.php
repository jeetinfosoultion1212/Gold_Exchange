...
// Gold Stock by Purity
$stock_sql = "SELECT id, stock_name, purity, current_stock FROM gold_stock WHERE company_id = $company_id ORDER BY purity DESC";
...
<div class="w-full">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-6">
        ...
        <div class="bg-white rounded-lg shadow-sm p-3 border border-gray-100 hover:shadow-md transition-shadow">
            <div class="flex flex-col">
                <div class="flex items-center justify-between">
                    <p class="text-xs font-medium text-gray-500 uppercase tracking-wider">Cash In Hand</p>
                    <div class="relative">
                        <button onclick="toggleDropdown('cash-dropdown')" class="text-gray-400 hover:text-gray-600 focus:outline-none">
                            <i class="fas fa-ellipsis-v text-xs"></i>
                        </button>
                        <div id="cash-dropdown" class="hidden absolute right-0 mt-1 w-40 bg-white rounded-md shadow-lg z-10 border border-gray-200">
                            <button onclick="openAddCashModal()" class="block w-full text-left px-3 py-2 text-xs text-gray-700 hover:bg-gray-100">
                                <i class="fas fa-plus text-green-600 mr-1"></i> Add Cash
                            </button>
                            <button onclick="openTransactionsModal('cash')" class="block w-full text-left px-3 py-2 text-xs text-gray-700 hover:bg-gray-100">
                                <i class="fas fa-history text-blue-600 mr-1"></i> View Transactions
                            </button>
                            <button onclick="openResetCashModal()" class="block w-full text-left px-3 py-2 text-xs text-gray-700 hover:bg-gray-100">
                                <i class="fas fa-redo text-red-600 mr-1"></i> Reset Cash
                            </button>
                        </div>
                    </div>
                </div>
                <div class="flex items-center justify-between mt-1">
                    <p class="text-lg font-bold text-gray-800">₹<?= number_format($balance_data['cash_balance'] ?? 0) ?></p>
        ...
        <div class="bg-white rounded-lg shadow-sm p-3 border border-gray-100 hover:shadow-md transition-shadow">
            <div class="flex flex-col">
                <div class="flex items-center justify-between">
                    <p class="text-xs font-medium text-gray-500 uppercase tracking-wider">Bank Balance</p>
                    <div class="relative">
                        <button onclick="toggleDropdown('bank-dropdown')" class="text-gray-400 hover:text-gray-600 focus:outline-none">
                            <i class="fas fa-ellipsis-v text-xs"></i>
                        </button>
                        <div id="bank-dropdown" class="hidden absolute right-0 mt-1 w-40 bg-white rounded-md shadow-lg z-10 border border-gray-200">
                            <button onclick="openAddBankModal()" class="block w-full text-left px-3 py-2 text-xs text-gray-700 hover:bg-gray-100">
                                <i class="fas fa-plus text-green-600 mr-1"></i> Add Bank
                            </button>
                            <button onclick="openTransactionsModal('bank')" class="block w-full text-left px-3 py-2 text-xs text-gray-700 hover:bg-gray-100">
                                <i class="fas fa-history text-blue-600 mr-1"></i> View Transactions
                            </button>
                            <button onclick="openResetBankModal()" class="block w-full text-left px-3 py-2 text-xs text-gray-700 hover:bg-gray-100">
                                <i class="fas fa-redo text-red-600 mr-1"></i> Reset Bank
                            </button>
                        </div>
                    </div>
                </div>
                <div class="flex items-center justify-between mt-1">
                    <p class="text-lg font-bold text-gray-800">₹<?= number_format($balance_data['bank_balance'] ?? 0) ?></p>
        ...
        <?php if (!empty($gold_stocks)): ?>
            <?php foreach ($gold_stocks as $stock): ?>
                <?php 
                    ...
                    $bg_class = $is_positive ? 'bg-emerald-50' : 'bg-red-50';
                    $icon = $is_positive ? 'fa-cubes' : 'fa-exclamation-circle';
                    $status_text = $is_positive ? 'In Hand' : 'Shortage';
                ?>
                <div class="bg-white rounded-lg shadow-sm p-3 border border-gray-100 hover:shadow-md transition-shadow">
                    <div class="flex flex-col">
                        <div class="flex justify-between items-start">
                            <div class="flex items-center gap-2">
                                <p class="text-xs font-medium text-gray-500 uppercase tracking-wider"><?= htmlspecialchars($stock['stock_name']) ?> (<?= $stock['purity'] ?>%)</p>
                                <span class="text-[10px] px-1.5 py-0.5 rounded-full font-medium <?= $bg_class ?> <?= $color_class ?>">
                                    <?= $status_text ?>
                                </span>
                            </div>
                            <div class="relative">
                                <button onclick="toggleDropdown('stock-dropdown-<?= $stock['id'] ?>')" class="text-gray-400 hover:text-gray-600 focus:outline-none">
                                    <i class="fas fa-ellipsis-v text-xs"></i>
                                </button>
                                <div id="stock-dropdown-<?= $stock['id'] ?>" class="hidden absolute right-0 mt-1 w-40 bg-white rounded-md shadow-lg z-10 border border-gray-200">
                                    <button onclick="openAddStockModal(<?= $stock['id'] ?>, '<?= htmlspecialchars($stock['stock_name']) ?>', <?= $stock['purity'] ?>)" class="block w-full text-left px-3 py-2 text-xs text-gray-700 hover:bg-gray-100">
                                        <i class="fas fa-plus text-green-600 mr-1"></i> Add Stock
                                    </button>
                                    <button onclick="openTransactionsModal('stock', <?= $stock['id'] ?>)" class="block w-full text-left px-3 py-2 text-xs text-gray-700 hover:bg-gray-100">
                                        <i class="fas fa-history text-blue-600 mr-1"></i> View Transactions
                                    </button>
                                    <button onclick="openResetStockModal(<?= $stock['id'] ?>, '<?= htmlspecialchars($stock['stock_name']) ?>', <?= $stock['purity'] ?>)" class="block w-full text-left px-3 py-2 text-xs text-gray-700 hover:bg-gray-100">
                                        <i class="fas fa-redo text-red-600 mr-1"></i> Reset Stock
                                    </button>
                                </div>
                            </div>
                        </div>
                        <div class="flex items-center justify-between mt-1">
                            <p class="text-lg font-bold text-gray-800"><?= number_format(abs($stock['current_stock']), 3) ?> <span class="text-xs font-normal text-gray-500">g</span></p>
                            <div class="p-1.5 <?= $bg_class ?> rounded-full">
                                <i class="fas <?= $icon ?> <?= $color_class ?> text-sm"></i>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 mb-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold text-gray-900">Daily Transaction Report</h2>
                <p class="text-sm text-gray-500">Summary of all activities per party</p>
            </div>
            
            <div class="flex flex-wrap items-end gap-3">
                <button onclick="openAddStockModalFree()" class="inline-flex items-center px-4 py-2 bg-amber-600 text-white text-sm font-medium rounded-md hover:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-amber-500 shadow-sm">
                    <i class="fas fa-plus mr-2"></i>Add Stock
                </button>
                <form class="flex flex-wrap items-end gap-3" method="GET">
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">From Date</label>
                        <input type="date" name="start_date" value="<?= $start_date ?>" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 sm:text-sm px-3 py-2 border">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">To Date</label>
                        <input type="date" name="end_date" value="<?= $end_date ?>" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 sm:text-sm px-3 py-2 border">
                    </div>
                    <button type="submit" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                        <i class="fas fa-filter mr-2"></i> Filter
                    </button>
                    <a href="export_report_pdf.php?start_date=<?= $start_date ?>&end_date=<?= $end_date ?>" target="_blank" class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md shadow-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
...
<div id="addStockModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        ...
        <form id="addStockForm" onsubmit="submitStockAction(event, 'add_stock')">
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-2">Stock Name <span class="text-red-500">*</span></label>
                <input type="text" name="stock_name" required placeholder="e.g., Fine Gold, 22K Gold, etc." class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-amber-500 focus:border-amber-500">
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-2">Purity (%) <span class="text-red-500">*</span></label>
                <input type="number" step="0.01" name="purity" id="addStockPurityInput" required min="0" max="100" placeholder="e.g., 99.90, 91.60, 75.00" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-amber-500 focus:border-amber-500">
                <p class="text-xs text-gray-500 mt-1">Enter purity percentage (0-100)</p>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-2">Weight (grams) <span class="text-red-500">*</span></label>
                <input type="number" step="0.001" name="amount" required placeholder="e.g., 100.500" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-amber-500 focus:border-amber-500">
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-2">Stock Type <span class="text-red-500">*</span></label>
                <select name="stock_type" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-amber-500 focus:border-amber-500">
                    <option value="">Select Type</option>
                    <option value="Cash">Cash (Kachha)</option>
                    <option value="Bank">Bank (Pakka)</option>
                </select>
                <p class="text-xs text-gray-500 mt-1">Cash = Kachha, Bank = Pakka</p>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-2">Notes</label>
                <textarea name="notes" rows="2" placeholder="Optional notes..." class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-amber-500 focus:border-amber-500"></textarea>
            </div>
            <input type="hidden" name="stock_id" id="addStockId">
            <div class="flex gap-2">
                <button type="submit" class="flex-1 bg-amber-600 text-white px-4 py-2 rounded-md hover:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-amber-500">
                    <i class="fas fa-plus mr-2"></i>Add Stock
                </button>
                <button type="button" onclick="closeModal('addStockModal')" class="flex-1 bg-gray-300 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-400">
                    Cancel
                </button>
            </div>
        </form>
