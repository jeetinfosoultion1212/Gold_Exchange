                ...
                <table class="rtable">
                    ...
                    <tbody>
                        ...
                        <tr class="bg-slate-50 font-black" id="exchanges-total-row">
                            ...
                            <td class="num" id="ex-total-rcv-cell"><?= number_format($ex_total_rcv, 3) ?> g</td>
                            <td class="num text-slate-500"><?= number_format($ex_avg_purity, 2) ?>%</td>
                            <td class="num text-amber-700">
                                <div class="inline-flex flex-wrap items-center gap-1.5 justify-end">
                                    ...
                                    <button type="button" id="transferAllFineBtn" class="ex-transfer-btn"
                                        ...
                                        data-rcv="<?= number_format($ex_total_rcv, 3, '.', '') ?>"
...
if (transferAllBtn && totalFineInput) {
    transferAllBtn.addEventListener('click', () => {
        ...
        const rcv = parseFloat(transferAllBtn.dataset.rcv) || 0;
        if (fine <= 0) {
            Swal.fire({ icon: 'warning', title: 'Enter fine weight', text: 'Set the total fine weight to transfer.' });
            return;
        }
        Swal.fire({
            title: 'Transfer to fine stock?',
            html: `
                <div class="text-left text-xs text-slate-600 space-y-2 mt-1">
                    <div class="flex justify-between gap-4 bg-slate-50 rounded-md px-3 py-2">
                        <span class="font-bold text-slate-500 uppercase">Total received</span>
                        <span class="font-black text-slate-800 tabular-nums">${rcv.toFixed(3)} g</span>
                    </div>
                    <div class="flex justify-between gap-4 bg-amber-50 rounded-md px-3 py-2 border border-amber-100">
                        <span class="font-bold text-amber-700 uppercase">Total fine</span>
                        <span class="font-black text-amber-800 tabular-nums">${fine.toFixed(3)} g</span>
                    </div>
                    <p class="text-[10px] text-slate-500 pt-1">Fine moves from MIX stock to fine gold stock for this period. Edit total fine above if refine result differs.</p>
                </div>
            `,
            icon: 'question',
            showCancelButton: true,
